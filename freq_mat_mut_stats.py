...
frequency_data_matrix.to_csv(f'/Users/arimaite/Documents/GitHub/SARS_CoV_2_variants/analysis_2024_KP.2_KP.3/statistic_KP_over_time/{location}/frequency_data_matrix_kp3_{location}_{date}.csv', header=True, index=True)
...
ax.set_xticklabels(df.transpose().index, fontsize=8, rotation=0, ha='right', va='center')


fig.savefig(f'/Users/arimaite/Documents/GitHub/SARS_CoV_2_variants/analysis_2024_KP.2_KP.3/statistic_KP_over_time/{location}/heatmapC23039G_G22599C_{location}_{date}.pdf', format="pdf", bbox_inches="tight")
...
for sample in df.columns:
    ...
    ax.plot(df.index, df[sample], label=explanatory_label, marker='o')  # 'o' adds points to the line plot
...
# Save the figure
fig.savefig(f'/Users/arimaite/Documents/GitHub/SARS_CoV_2_variants/analysis_2024_KP.2_KP.3/statistic_KP_over_time/{location}/lineplotC23039G_G22599C_{location}_{date}.pdf', format="pdf", bbox_inches="tight")
...
mut_last_2_weeks = frequency_data_matrix.loc[:, frequency_data_matrix.columns >= two_weeks_ago]
...
mut_last_6_weeks = frequency_data_matrix.loc[:, frequency_data_matrix.columns >= six_weeks_ago]
...
mut_last_3_months = frequency_data_matrix.loc[:, frequency_data_matrix.columns >= three_months_ago]
...
mut_last_6_months = frequency_data_matrix.loc[:, frequency_data_matrix.columns >= six_months_ago]
...
for i, mutation in enumerate(frequency_data_matrix.index):
    dict_mut[f'df_mut_{mutation}'] = pd.concat([round(results_2_weeks.loc[mutation,:],3), round(results_6_weeks.loc[mutation,:],3), round(results_12_weeks.loc[mutation,:],3), round(results_24_weeks.loc[mutation,:],3)],
                                               keys=['2weeks', '6weeks', '12weeks','24weeks'], axis=0)
combined_df = pd.concat(dict_mut, axis=0).reset_index()
combined_df.columns = ['mutation', 'time', 'statistic', 'value']

print(combined_df)
combined_df.to_csv(f'/Users/arimaite/Documents/GitHub/SARS_CoV_2_variants/analysis_2024_KP.2_KP.3/statistic_KP_over_time/{location}/mutations_statistics_C23039G_G22599C_{location}_{date}.csv', header=True, index=False)