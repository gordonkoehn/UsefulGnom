...
def extract_mutation_position_and_nt(datamatrix_dir):
    ...
    extracted_data = [(re.search(pattern, mutation).group(1), re.search(pattern, mutation).group(2)) for mutation in
                      datamatrix['mut']]
...
def load_convert(coverage_path, pos_mut):
    ...
    with gzip.open(coverage_path, 'rt') as file:
        ...
        df = pd.read_csv(file, delimiter='\t', usecols=[1, 2, 3, 4, 5], header=None, index_col=None)[3:]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Process basecnt.tsv.gz files and make mutation confidence matrix')
    parser.add_argument('basecnt_tsv_dir', help='input directory containing basecnt tsv files: it should be ...'
                                                 '/cluster/project/pangolin/work-vp-test/results/*/*/alignments/basecnt.tsv.gz')
    parser.add_argument('datamatrix_dir', help='input directory of the datamatrix csv file')
