...
def parse_args():
    ...
    parser.add_argument(
        "-p", dest="makeplots", help="Output plots.", action="store_true"
    )
...
def get_samples_paths(
    main_samples_path="/cluster/project/pangolin/working/samples",
    samplestsv="/cluster/project/pangolin/working/samples.tsv",
):
...
def main():
    ...
    # make amplicons df
    if args.v:
        print("Loading primers bedfile.")
    amplicons_df = make_amplicons_df(load_bedfile(bedfile_addr))

    # read list of samples
    if args.v:
        print("Reading list of coverage files.")
    sam_list = get_samples_paths(samp_path, samp_file)

    # iterate through list of samples
    if args.v:
        print("Loading and parsing coverage files.")
    all_covs = []
    indexes = []
    i = 1
    for sam in sam_list:
        if args.v:
            print("Parsing coverage file {}/{}".format(i, len(sam_list)), end="\r")
        try:
            ...
            if args.v:
                print("WARNING: file {} not found.".format(sam))
        #         all_covs.append([])
        i += 1
