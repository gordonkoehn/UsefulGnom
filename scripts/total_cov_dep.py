...
def extract_mutation_position(datamatrix_dir):
    ...
    extracted_data = [re.search(pattern, mutation).group() for mutation in datamatrix['mut']]
...
def extract_sample_ID(timeline_file_dir):
    ...
    timeline_file = pd.read_csv(timeline_file_dir, sep = '\t', usecols = ['sample', 'proto', 'date', 'location'],
                                encoding='utf-8')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Process coverage.tsv.gz files and make position coverage matrix')
    parser.add_argument('coverage_tsv_dir', help='input directory containing general coverage.tsv.gz files: it should be: '
                                                 '/cluster/project/pangolin/work-vp-test/results/*/*/alignments/coverage.tsv.gz')
    parser.add_argument('datamatrix_dir', help='input directory of the datamatrix csv file with relevant mutations')
    parser.add_argument('timeline_file_dir', help='input directory of the timeline tsv file')

    parser.add_argument('output_file', help='output CSV file: total coverage of that position over the time')

    args = parser.parse_args()

    main(args.coverage_tsv_dir, args.datamatrix_dir, args.timeline_file_dir, args.output_file)